...
function distributeMessage(message_list: Array<string>): string {
    if(message_list.length === 0) return "zero";
...
const HowNerd : command.Command = {
    data: new SlashCommandBuilder()
        .setName('how_nerd')
		.setDescription('Do you know how nerdy you really are? Because I do.'),
    async execute(interaction, client) {
        ...
        if(percentage < 10)
            ...
            message += distributeMessage(nerd_data.messages[5]);
        else message += distributeMessage(nerd_data.messages[6]); 
